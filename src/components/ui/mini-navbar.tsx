    ...
    const resumeButtonElement = (
        <div className="relative group w-full sm:w-auto">
            ...
            <a
                ...
                className="relative z-10 px-4 py-2 sm:px-3 text-xs sm:text-sm font-semibold text-white bg-gradient-to-br from-accent to-accent-secondary rounded-full hover:opacity-90 transition-all duration-200 w-full sm:w-auto block text-center"
            >
                Resume
            </a>
        </div>
        ...
        <header className={`fixed top-6 left-1/2 transform -translate-x-1/2 z-50
            ...
            <div className={`sm:hidden flex flex-col items-center w-full transition-all ease-in-out duration-300 overflow-hidden
                ...
                <nav className="flex flex-col items-center space-y-4 text-base w-full">
                    {navLinksData.map((link) => (
                        <a key={link.href} href={link.href} className="text-gray-300 hover:text-white transition-colors w-full text-center" onClick={() => setIsOpen(false)}>
                            {link.label}
                        </a>
                    ))}
                </nav>
